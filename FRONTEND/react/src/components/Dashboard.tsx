...
const Dashboard: React.FC<DashboardProps> = ({ currentUser }) => {
  ...
  const [averageCompletionTime, setAverageCompletionTime] =
    useState<AverageCompletionTimeData | null>(null);
  const [weeklyCompletionRate, setWeeklyCompletionRate] =
    useState<WeeklyCompletionRateData | null>(null);
  ...
  const fetchAllMetrics = async () => {
    ...
    try {
      ...
      const responses = await Promise.all([
        ...
        axios.get(`${API_URL_METRICS}/top-tags`, {
          params: { ...userIdParams.params, count: 5 },
        }),
        axios.get(`${API_URL_METRICS}/completed-by-day`, {
          params: { ...userIdParams.params, days: 7 },
        }),
        axios.get(`${API_URL_METRICS}/average-completion-time`, userIdParams),
        axios.get(`${API_URL_METRICS}/weekly-completion-rate`, userIdParams),
      ]);
      ...
      setAverageCompletionTime(responses[4].data);
      setWeeklyCompletionRate(responses[5].data);
    } catch (err: any) {
  ...
  const formatDuration = (totalSeconds: number | null | undefined): string => {
    ...
    if (seconds > 0 && days === 0 && hours === 0) parts.push(`${seconds} seg`);
  ...
  return (
    <div className="container mt-4">
      <h2 className="mb-3">
        Dashboard de Produtividade de {currentUser.username}
      </h2>
      ...
      <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
          ...
          <div className="card h-100 shadow-sm">
            <div className="card-body">
              <h5 className="card-title">
                <i className="bi bi-bar-chart-steps me-2"></i>Tarefas por Status
              </h5>
                ...
                <ul className="list-group list-group-flush">
                  <li className="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <span className="badge bg-warning rounded-pill">
                      {tasksByStatus.pendente || 0}
                    </span>
                  </li>
                  <li className="list-group-item d-flex justify-content-between align-items-center">
                    Em Andamento{' '}
                    <span className="badge bg-primary rounded-pill">
                      {tasksByStatus['em andamento'] || 0}
                    </span>
                  </li>
                  <li className="list-group-item d-flex justify-content-between align-items-center">
                    Concluída{' '}
                    <span className="badge bg-success rounded-pill">
                      {tasksByStatus.concluída || 0}
                    </span>
                  </li>
                </ul>
              ) : (
        ...
        <div className="col">
          <div className="card h-100 shadow-sm">
            <div className="card-body text-center">
              <h5 className="card-title">
                <i className="bi bi-calendar-plus me-2"></i>Criadas Hoje
              </h5>
        ...
        <div className="col">
          <div className="card h-100 shadow-sm">
            <div className="card-body">
              <h5 className="card-title">
                <i className="bi bi-tags me-2"></i>Top Tags
              </h5>
              {topTags.length > 0 ? (
                <ul className="list-group list-group-flush">
                  {topTags.map((item) => (
        ...
        <div className="col">
          <div className="card h-100 shadow-sm">
            <div className="card-body text-center">
              <h5 className="card-title">
                <i className="bi bi-clock-history me-2"></i>Tempo Médio de
                Conclusão
              </h5>
              {averageCompletionTime &&
              averageCompletionTime.average_seconds !== null ? (
                <>
                  <p className="display-6 fw-bold">
                    {formatDuration(averageCompletionTime.average_seconds)}
                  </p>
                  <small className="text-muted">
                    Baseado em {averageCompletionTime.total_completed} tarefa(s)
                    concluída(s)
                  </small>
                </>
              ) : (
                <p className="text-muted mt-3">
                  {averageCompletionTime?.message || 'Calculando...'}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="col">
          <div className="card h-100 shadow-sm">
            <div className="card-body text-center">
              <h5 className="card-title">
                <i className="bi bi-check2-circle me-2"></i>Taxa de Conclusão
                Semanal
              </h5>
              {weeklyCompletionRate && weeklyCompletionRate.rate !== null ? (
                <>
                  <p className="display-4 fw-bold">
                    {(weeklyCompletionRate.rate * 100).toFixed(0)}%
                  </p>
                  <small className="text-muted">
                    {weeklyCompletionRate.tasks_completed_last_7_days} de{' '}
                    {weeklyCompletionRate.tasks_created_last_7_days} tarefas
                    criadas nos últimos 7 dias.
                  </small>
                </>
              ) : (
                <p className="text-muted mt-3">
                  {weeklyCompletionRate?.message || 'Calculando...'}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="col">
          <div className="card h-100 shadow-sm">
            <div className="card-body">
              <h5 className="card-title">
                <i className="bi bi-calendar-check me-2"></i>Concluídas por Dia
              </h5>
              {completedTasksByDay.length > 0 ? (
                <ul className="list-group list-group-flush">
                  {completedTasksByDay.slice(0, 7).map((item) => (
                    <li
                      key={item.date}
                      className="list-group-item d-flex justify-content-between align-items-center"
                    >
                ...
                <p className="text-muted">
                  Nenhuma tarefa concluída nos últimos dias.
                </p>
              )}
            </div>
          </div>
